...
export function EnhancedDumpViewer({
  ...
  const [filterType, setFilterType] = useState<'all' | 'exceptions' | 'data'>('all')
  const containerRef = useRef<HTMLDivElement>(null)

  // Process dumps to detect exceptions
  const processedDumps = useMemo(() => {
    return dumps.map((dump) => {
      const parsedException = ExceptionParser.parse(dump.payload)
      return {
        dump,
        isException: !!parsedException,
        parsedException
      } as ProcessedDump
    })
  }, [dumps])

  // Filter dumps based on type
  const filteredDumps = useMemo(() => {
    switch (filterType) {
      case 'exceptions':
        return processedDumps.filter((d) => d.isException)
      case 'data':
        return processedDumps.filter((d) => !d.isException)
      default:
        return processedDumps
    }
  }, [processedDumps, filterType])
  ...
  }, [dumps.length, autoScroll])

  const dumpStats = useMemo(() => {
    const stats = {
      total: dumps.length,
      exceptions: processedDumps.filter((d) => d.isException).length,
      data: processedDumps.filter((d) => !d.isException).length,
    ...
    processedDumps.forEach(({ dump, parsedException }) => {
      // By server
      const server = getServerById(dump.serverId)
      const serverName = server?.name || 'Unknown Server'
      stats.byServer[serverName] = (stats.byServer[serverName] || 0) + 1

      // By framework (for exceptions)
      if (parsedException?.framework) {
        stats.byFramework[parsedException.framework] =
          (stats.byFramework[parsedException.framework] || 0) + 1
      }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-4">
                ...
                {/* Exception Count */}
                {dumpStats.exceptions > 0 && (
                  <div className="flex items-center space-x-1 text-sm">
                    <span className="text-red-600 dark:text-red-400">🔥</span>
                    <span className="font-medium text-red-600 dark:text-red-400">
                      {dumpStats.exceptions}
                    </span>
                    <span className="text-slate-500 dark:text-slate-400">exceptions</span>
                  </div>
                )}
                ...
                <div className="flex items-center space-x-1 text-sm">
                  ...
                  <span className="text-slate-500 dark:text-slate-400">data dumps</span>
                </div>

                {/* Recent activity */}
                {dumpStats.recentActivity > 0 && (
                  <div className="flex items-center space-x-1 text-xs px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 rounded-full">
                    ...
                    <span>{dumpStats.recentActivity} new</span>
                  </div>
                )}
              </div>

              {/* Framework breakdown */}
              {Object.keys(dumpStats.byFramework).length > 0 && (
                <div className="flex items-center space-x-2 pl-4 border-l border-slate-300 dark:border-slate-600">
                  {Object.entries(dumpStats.byFramework).map(([framework, count]) => (
                    <div
                      key={framework}
      ...
      >
        <div className="max-w-7xl mx-auto py-6 px-6">
          ...
          >
            {filteredDumps.map(({ dump, isException, parsedException }) => {
              const server = getServerById(dump.serverId)
              const isExpanded = expandedDump === dump.id
              const isNew = newDumpIds.has(dump.id)

              // Use ExceptionDumpItem for exceptions, regular DumpItem for others
              if (isException && parsedException) {
                return (
                  <ExceptionDumpItem
                    key={dump.id}
                    dump={dump}
          ...
          {filteredDumps.length === 0 && filterType !== 'all' && (
            <div className="text-center py-16">
              <div className="text-6xl mb-4">{filterType === 'exceptions' ? '🔍' : '📊'}</div>
              <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-2">
                No {filterType === 'exceptions' ? 'exceptions' : 'data dumps'} found
              </h3>
              <p className="text-slate-600 dark:text-slate-400">
                Try changing the filter or wait for new dumps to arrive
              </p>
              <button
                onClick={() => setFilterType('all')}
  ...
  const icons: Record<string, string> = {
    laravel: (
      <svg
        ...
        viewBox="0 0 640 640"
        className={'w-5 h-5 fill-black dark:fill-white stroke-2'}
      >
    ...
    react: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 640 640"
        className={'h-5 w-5 fill-black dark:fill-white stroke-2'}
      >
    ...
    js: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 640 640"
        className={'h-3 w-5 fill-black dark:fill-white stroke-2'}
      >
